...
FRDS_INCIDENTS = [
    ...
    },
    {
        ...
        'content': 'Spanner Vector Search connection timeouts happen when the database connection pool is exhausted or when network latency between the application tier and Spanner exceeds the configured timeout threshold of 30 seconds. Resolution: increase the connection pool size in the frds-config.yaml file, check network routes between application servers and Spanner instances, and verify that the Spanner instance is not under heavy load. Monitor the connection_pool_exhausted metric in Grafana.',
        'metadata': {
            'category': 'Database',
            'severity': 'Critical',
    ...
    },
    {
        ...
        'content': 'A 503 Service Unavailable error from the FRDS API gateway indicates that the backend services are not responding. This can be caused by: (1) Backend pod crash loops in Kubernetes, (2) Health check failures, (3) Resource limits being exceeded. To troubleshoot: check pod status with kubectl get pods -n frds, review pod logs with kubectl logs, verify resource requests and limits in the deployment manifest, and check if horizontal pod autoscaler is functioning correctly.',
        'metadata': {
            'category': 'API',
            'severity': 'Critical',
    ...
    },
    {
        ...
        'content': 'PSPD FRDS user guides and documentation are available at the internal documentation portal: pspd-frds-help-dev.cbp.dhs.gov. The portal contains: Getting Started guide, API reference documentation, Troubleshooting playbooks, Release notes, and Administrator configuration guides. Access requires valid CBP network credentials. For access issues, contact the FRDS support team at frds-support@cbp.dhs.gov.',
        'metadata': {
            'category': 'Documentation',
            'severity': 'Low',
            'resolution': 'Visit pspd-frds-help-dev.cbp.dhs.gov for documentation.',
            'error_code': 'DOC-001',
        },
    },
    {
    ...
    },
    {
        ...
        'content': 'FRDS incident data synchronization from source systems may experience delays exceeding the 15-minute SLA. Common causes include: source system API rate limiting, network congestion during peak hours, or the sync worker queue backlog exceeding 1000 items. Troubleshooting: check the sync worker queue depth in the FRDS admin dashboard, review source system API response times, verify the sync schedule in cron configuration, and check for any failed sync jobs in the job history.',
        'metadata': {
            'category': 'Data Sync',
            'severity': 'Medium',
    ...
    },
    {
        ...
        'content': 'To perform a rollback of a FRDS deployment: (1) Identify the previous stable release version from the release history, (2) Execute kubectl rollout undo deployment/frds-app -n frds to revert to the previous version, (3) Verify the rollback with kubectl rollout status, (4) Run the smoke test suite with pytest tests/smoke/ -v, (5) Update the deployment tracker in Jira. For database migrations that need reversal, use python manage.py migrate chat <previous_migration_number> to reverse the schema changes.',
        'metadata': {
            'category': 'Deployment',
            'severity': 'High',
    ...
    },
    {
        ...
        'content': 'SSL handshake failures with upstream services typically occur when the upstream service updates its TLS configuration or when there is a certificate chain issue. Debug with: openssl s_client -connect upstream-host:443 -showcerts. Common fixes: update the CA bundle in the FRDS trust store, verify that the intermediate certificates are included in the chain, and check that TLS 1.2 or higher is configured. Update the trust store path in frds-ssl-config.yaml.',
        'metadata': {
            'category': 'Networking',
            'severity': 'High',
